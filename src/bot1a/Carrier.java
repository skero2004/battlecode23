...
public class Carrier {
	...
	static void runCarrier(RobotController rc) throws GameActionException {
		...
		if (anchorMode) {
			if (islandLoc == null) {
				for (int i = Communication.STARTING_ISLAND_IDX; i < Communication.STARTING_ISLAND_IDX
						+ GameConstants.MAX_NUMBER_ISLANDS; i++) {
			...
			if (total == 0) {
				...
				if (wellLoc != null && !rc.getLocation().isAdjacentTo(wellLoc))
