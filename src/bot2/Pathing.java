        ...
        switch (type) {
            case WELL_AD:
                for (WellInfo w : rc.senseNearbyWells(ResourceType.ADAMANTIUM)) {
                    // if (Communication.setItem(rc, new Well(LocationType.WELL_AD, new
                    // Vec2D(w.getMapLocation()), w.isUpgraded())))
            ...
            case ISLAND:
                for (int i : rc.senseNearbyIslands()) {
                    for (MapLocation l : rc.senseNearbyIslandLocations(i)) {
                        // if (Communication.setItem(rc, new Island(LocationType.ISLAND, new Vec2D(l),
                        // rc.senseTeamOccupyingIsland(i) == rc.getTeam(), false, false)))
            ...
            case HEADQUARTERS:
                for (RobotInfo r : rc.senseNearbyRobots()) {
                    if (r.getTeam() == rc.getTeam() && r.getType() == RobotType.HEADQUARTERS) {
                        // if (Communication.setItem(rc, new Headquarters(LocationType.HEADQUARTERS, new
                        // Vec2D(r.getLocation()))))
